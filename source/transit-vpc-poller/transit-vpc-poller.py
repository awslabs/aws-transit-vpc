    ...
    try:
      ...
      for vgw in vgws['VpnGateways']:
        ...
        if processed_vgw:
          ...
          num_failed_attempts = 0 
          break
    # Poor man's self-throttling using exponential backoff
    # Encountered a server exception while processing a VGW, exponentially backoff before proceeding to next region
    except botocore.exceptions.ServerError as e:
        num_failed_attempts = num_failed_attempts + 1
        backoff = (2 ** num_failed_attempts) * 2
        log.info('Encountered a server exception, sleeping for %d seconds before proceeding', backoff)
        time.sleep(backoff)
        pass
