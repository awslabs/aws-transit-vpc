...
echo "Building transit-vpc-solution-helper ZIP file"
...
echo "pip install $deployment_dir/../source/transit-vpc-solution-helper/. --target=$deployment_dir/dist/env/lib/python3.7/site-packages/ --upgrade --upgrade-strategy only-if-needed"
pip install $deployment_dir/../source/transit-vpc-solution-helper/. --target=$deployment_dir/dist/env/lib/python3.7/site-packages/ --upgrade --upgrade-strategy only-if-needed
# fail build if pip install fails
instl_status=$?
if [ ${instl_status} != '0' ]; then
  echo "pip install solution helper status: ${instl_status}"
  exit ${instl_status}
fi
cd $deployment_dir/dist/env/lib/python3.7/site-packages/
zip -r9 $deployment_dir/dist/transit-vpc-solution-helper.zip .
cd $deployment_dir/dist
zip -q -d transit-vpc-solution-helper.zip pip*
zip -q -d transit-vpc-solution-helper.zip easy*
...
echo "source env/bin/activate"
source env/bin/activate
echo "pip install $deployment_dir/../source/transit-vpc-push-cisco-config/. --target=$deployment_dir/dist/env/lib/python3.7/site-packages/ --upgrade --upgrade-strategy only-if-needed"
pip install $deployment_dir/../source/transit-vpc-push-cisco-config/. --target=$deployment_dir/dist/env/lib/python3.7/site-packages/ --upgrade --upgrade-strategy only-if-needed
